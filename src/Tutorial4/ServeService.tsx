...
const ServeService: React.FC = () => {
  ...
  const [serveService, setServeService] =
    useState<ROSLIB.Service<{ a: number; b: number }, { sum: number }>>()
  ...
  return (
    <div>
      {answer ? (
        <p>
          {' '}
          {serveService?.name} is called. The answer is {answer}.
        </p>
      ) : (
        <p>Not called. Please call the "/add_two_ints" service.</p>
      )}
    </div>
  )
}
